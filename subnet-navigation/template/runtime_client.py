...
def runtime_timeout(config: Any) -> float:
    try:
        return float(getattr(config.neuron, "runtime_timeout", 10.0) or 10.0)
    except Exception:
        return 10.0
...
def try_post_json(base_url: str, path: str, payload: dict[str, Any], timeout_s: float) -> dict[str, Any] | None:
    if not base_url:
        return None
    try:
        ...
        bt.logging.warning(f"navigation runtime {path} failed: {e}")
        return None
